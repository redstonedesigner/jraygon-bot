...
class Lobby(vbu.Cog):
    ...
    async def create(self, ctx: vbu.Context, lobby: discord.VoiceChannel, private: discord.VoiceChannel, join_requests: discord.TextChannel, allowed_role: discord.Role):
        """
        Create a new lobby channel.
        """
        success = await lobby_vc.create(ctx.guild.id, lobby.id, private.id, join_requests.id, str(allowed_role.id))
        if success:
    ...
    @commands.defer()
    @commands.has_guild_permissions(manage_channels=True)
    async def add_role(self, ctx: vbu.Context, lobby: discord.VoiceChannel, role: discord.Role):
        """
    ...
    @commands.defer()
    @commands.has_guild_permissions(manage_channels=True)
    async def remove_role(self, ctx: vbu.Context, lobby: discord.VoiceChannel, role: discord.Role):
        """
    ...
    async def info(self, ctx: vbu.Context, lobby: discord.VoiceChannel):
        ...
        if not success:
            msg = "There is no lobby system associated with that Voice Channel."
            await respond(ctx, msg)
        else:
    ...
    async def handle_lobby_join(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        if after.channel is None:
            pending_request = await lobby_vc.get_request_by_member_id(member.id)
            if pending_request:
                success, data, role_ids = await lobby_vc.get(member.guild.id, before.channel.id)
                if not success:
                    return
                prior_request_channel = member.guild.get_channel(data['request_channel_id'])
                request_msg = await prior_request_channel.fetch_message(pending_request[0]['message_id'])
                await request_msg.delete()
                await lobby_vc.delete_request(member.id)
            return
        success, data, role_ids = await lobby_vc.get(member.guild.id, after.channel.id)
        if not success:
            return
        ...
        components = discord.ui.MessageComponents(
            discord.ui.ActionRow(
                discord.ui.Button(
                    label='Admit', custom_id='LobbyVC_Admit', style=discord.ButtonStyle.green
                ),
                discord.ui.Button(
                    label='Reject', custom_id='LobbyVC_Reject', style=discord.ButtonStyle.red
                )
            )
        )
        request_channel = member.guild.get_channel(data['request_channel_id'])
        request_msg = await request_channel.send(embed=request_embed, components=components)
        await lobby_vc.create_request(data['lobby_channel_id'], request_msg.id, member.id)
    ...
    async def handle_component(self, interaction: discord.Interaction):
        ...
        join_request = await lobby_vc.get_request_by_message_id(interaction.message.id)
        if not join_request:
            msg = "Unable to receive join request information.  OG MSG ID : "+str(interaction.message.id)
            await interaction.followup.send(content=msg)
            return
        success, data, role_ids = await lobby_vc.get(interaction.guild.id, join_request[0]['lobby_vc_id'])
        if not success:
            msg = "An error occurred."
        else:
            allowed_roles = [interaction.guild.get_role(int(role_id)) for role_id in role_ids]
            can_manage = False
            for i in allowed_roles:
                if i in interaction.user.roles:
                    ...
                    break
            if not can_manage:
                await interaction.followup.send(content="You do not have permission to do that!", ephemeral=True)
                return
            member = interaction.guild.get_member(join_request[0]['user_id'])
            reason_prefix = f"[{interaction.user.name}#{interaction.user.discriminator}]"
            if interaction.component.custom_id == 'LobbyVC_Admit':
                await lobby_vc.delete_request(member.id)
                channel = interaction.guild.get_channel(data['private_channel_id'])
                await member.move_to(channel=channel, reason=f"{reason_prefix} Join request accepted.")
                ...
                await lobby_vc.delete_request(member.id)
                await member.move_to(channel=None, reason=f"{reason_prefix} Join request rejected.")
        ...
        await interaction.message.edit(embeds=interaction.message.embeds, components=None)
