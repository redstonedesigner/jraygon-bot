...
class Reports(vbu.Cog):
    ...
    async def _context_command_report_user(self, ctx: vbu.SlashContext, user: discord.Member):
    ...
    @commands.guild_only()
    async def report_user(self, ctx: vbu.Context, user: discord.Member, reason: str = None):
        ...
        if reason is None:
            ...
            try:
                ...
                await ctx.interaction.followup.send("Please check your messages to proceed.")
            except discord.Forbidden:
                await ctx.interaction.followup.send("Error: You need to enable DMs from server members to use this context command.")
                return
            try:
                check = lambda m: m.author.id == ctx.author.id and m.channel.id == ctx.author.dm_channel.id
    ...
    async def _context_command_report_message(self, ctx: vbu.SlashContext, message: discord.Message):
