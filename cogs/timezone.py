...
class TimezoneInfo(vbu.Cog):
    ...
    async def timezone_set(self, ctx: vbu.Context, *, offset: str = None):
        ...
        try:
            ...
            return await ctx.send(
                f"I can't work out what timezone you're referring to - please run this command again to try later, or go to the website (`{ctx.clean_prefix}info`) and I can work it out automatically."
            )
    ...
    async def timezone_get(
        self,
        ctx: vbu.Context,
        target: typing.Union[discord.Member, str] = None,
    ):
        """
        ...
        if not target_is_timezone:
            async with vbu.Database() as db:
                rows = await db(
                    "SELECT timezone_name FROM user_settings WHERE user_id=$1",
                    target.id,
                )
            if not rows or rows[0]["timezone_name"] is None:
                return await ctx.send(
